...
다음을 수행하세요:
1. 증상과 관련된 DB 상태를 SQL로 검증 (Supabase MCP execute_sql 또는 CLI)
2. 관련 RPC 함수 실행하여 정상 동작 여부 확인
3. RLS 정책이 올바른지 확인: SELECT policyname, cmd, qual FROM pg_policies WHERE tablename = '{table}';
...
종합된 Root cause를 기반으로:
- **Backend change needed: Y** → /preflight → Generator (13-step) → /evaluate
...
Sprint Contract 작성 후, 2명의 에이전트를 스폰하여 contract를 교차 검증:
...
### STEP 3: Generator
13-step 실행. Server Boundary Checkpoint 필수.
